...
mean absolute NDCG@10 gap is $1.6\%$ across the six TREC-DL cells
and $1.9$--$4.5\%$ across the eight BEIR sets at three model
scales, and we \emph{exceed} the paper's reported numbers on
several individual cells (TREC-DL/Flan-T5-XL/DL19,
TREC-DL/Flan-T5-Large/DL20, BEIR-SciFact/Flan-T5-Large,
BEIR-Touch\'e-2020 at both Flan-T5-XL and Flan-UL2). We surface
...
(iv) the spectral MDS anchor is consistently mediocre on the
8 BEIR sets we ran anchor ablations on -- it never wins
(Top-3 wins 5/8, Random wins 3/8) but also never finishes last,
contradicting the paper's reported BEIR-aggregate ordering; (v) the contrastive-anchor mechanism
...
and the paper's own reported Flan-UL2 (0.7206 from Table~1
RG-YN+GCCP row); and (vi) on the
...
\begin{itemize}[leftmargin=1.5em,nosep]
    \item A paper-then-code reimplementation: we coded each of the
          ...
          and $1.9$--$4.5\%$ across the 24 BEIR cells, exceeding
          the paper on five cells (TREC-DL/Flan-T5-XL/DL19,
          TREC-DL/Flan-T5-Large/DL20, BEIR-SciFact/Flan-T5-Large,
          and BEIR-Touch\'e-2020 at both Flan-T5-XL and Flan-UL2).
    ...
    \item Four extensions absent from the original paper:
          ...
          authors' RG-S$(0,4)$ classifier; and (iv) a RankGPT-listwise
          comparison at matched model scale.
    ...
    \item Aggregation and anchor-construction ablations across 8
          ...
          not generalize. Spectral MDS anchor construction is
          consistently mediocre across all 8 BEIR sets (third or
          fourth in every set we tested) -- a stronger negative
...
to our 2-component PAGC). The average absolute gap is 4.5\% at
T5-Large, 1.9\% at T5-XL, and 3.3\% at UL2. We exceed the paper on
SciFact (T5-Large, $+4.2\%$) and Touch\'e-2020 (both T5-XL,
$+2.1\%$, and UL2, $+7.9\%$); the remaining gaps are negative but
not consistent in magnitude across datasets, suggesting per-dataset
preprocessing differences rather than a single bug.
...
exceeds the paper's published PAGC$+$BM25 number (Table~1
RG-YN+GCCP row, T5-XL: $0.6969$ on DL19, $0.6810$ on DL20) by
$+2.2$ and $+3.7$~pts respectively. On SciFact, PAGC$+$E5
($0.7176$) is $+3.4$~pts over our PAGC$+$BM25 ($0.6840$) and
$+3.2$~pts over the paper's T5-XL number ($0.6854$).
...
\paragraph{Finding.} The two-component PAGC numbers we report
match the paper's three-component PAGC-QSG within $\pm 0.02$ NDCG@10
on Flan-T5-Large, despite using only RG-YN $+$ GCCP rather than QG
$+$ RG-S(0,4) $+$ GCCP. As a listwise comparison the paper itself
reports PAGC-QSG (Flan-T5-Large) beating its own listwise/pairwise
baselines (RankGPT $0.6690$, PRP-Allpair $0.6646$,
Setwise-Heapsort $0.6700$ on DL19) by $+2$ to $+4$~pts. Our PAGC
($0.6852$) sits within that bracket, slightly above all of the
paper's listwise/pairwise baselines and slightly below its
PAGC-QSG ($0.6973$). \textbf{Our own re-run of RankGPT-listwise
via \texttt{llm-rankers} produces much weaker numbers than the
paper reports under the same backbone}, which we leave as an open
discrepancy: at fixed Flan-T5 scale our pointwise-with-contrastive
PAGC is comfortably above our re-run RankGPT, but is also above the
paper's reported RankGPT, so the gap may be a re-implementation
issue on the listwise side rather than a fundamental advantage of
PAGC at this scale.
...
of the 8 sets, but also never lands last -- it is consistently
the second or third anchor across sets.} Top-3 composite wins 5/8
...
exception (GCCP+Top slightly beats GCCP+Spectral); we now find
that on \emph{every single one of the 8 BEIR sets we ran},
Spectral MDS loses to at least one simpler anchor, but also
\emph{never finishes last} -- it is consistently a middle-of-the-pack
choice. The picture is therefore ``Spectral is consistently
mediocre across sets,'' not ``Spectral is bad.'' The aggregate
ordering on our 8 sets (Top-3 $>$ Top-1 $>$ Spectral $>$ Random)
is nonetheless the \emph{opposite} of the paper's reported
aggregate (Spectral $>$ Top $>$ Random).
...
We have sent an email to the corresponding author at NUDT with
this specific question; if a response arrives before camera-ready
we will fold it in. The email itself is included in the supplementary
material.
...
``GCCP+PAGC always boosts pointwise'' framing.
...
\begin{enumerate}[leftmargin=1.5em,nosep]
    ...
    \item The \textbf{spectral MDS anchor is consistently mediocre}
          on the 8 BEIR sets we ran anchor ablations on
          (Table~\ref{tab:beir-anchor}): it never wins on any set,
          but it also never finishes last -- Top-3 wins 5/8,
          Random wins 3/8, and Spectral lands in the middle of the
          four anchors on every set. Aggregate ordering across all
          8 sets is Top-3 (0.4581) $>$ Top-1 (0.4512) $>$ Spectral
          (0.4446) $>$ Random (0.4423), the \emph{opposite} of the
          paper's reported aggregate Spectral (0.4471) $>$ Top
          (0.4346) $>$ Random (0.4253). Our Spectral number matches
          the paper's within $0.003$; our Top and Random are
          systematically $+0.017$ above the paper's, suggesting
          different operationalizations -- we sent the original
          authors an email asking which (Section~\ref{sec:anchor-ablation}).
    ...
    \item Decoder-only LLMs work with our chat-template adaptation
          ...
          Flan-UL2 (0.7095) and the paper's own Flan-UL2
          ($0.7206$, Table~1 RG-YN+GCCP row) on DL19.
